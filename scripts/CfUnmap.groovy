...
USAGE = '''
grails cf-unmap <url> [--appname]
'''

target(cfUnmap: 'Unregister a url from an application') {
	depends cfInit
	...
	doWithTryCatch {
		...
		if (!application.uris.contains(uri)) {
			errorAndDie "You can only unmap a previously registered URL.\nRegistered URLs are $application.uris"
		}
